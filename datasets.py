...
class ImageDataset(object):
    def __init__(self, Xs, Ys, split=[0.8, 0.1, 0.1],
        ...
        self.all_imgs = Xs
        self.all_labels = Ys
        n_idxs = len(self.all_imgs)
        idxs = range(n_idxs)
        rand_idxs = np.random.permutation(idxs)
        self.all_imgs = [self.all_imgs[idx] for idx in rand_idxs]
        self.all_labels = [self.all_labels[idx] for idx in rand_idxs]
    ...
    @property
    def train(self):
        imgs = [self.all_imgs[idx] for idx in self.train_idxs]
        labels = [self.all_labels[idx] for idx in self.train_idxs]
        return DatasetSplit(imgs, labels)

    @property
    def valid(self):
        imgs = [self.all_imgs[idx] for idx in self.valid_idxs]
        labels = [self.all_labels[idx] for idx in self.valid_idxs]
        return DatasetSplit(imgs, labels)

    @property
    def test(self):
        imgs = [self.all_imgs[idx] for idx in self.test_idxs]
        labels = [self.all_labels[idx] for idx in self.test_idxs]
        return DatasetSplit(imgs, labels)

    def mean(self):
        return np.mean(self.all_imgs, axis=0)

    def std(self):
        return np.std(self.all_imgs, axis=0)
